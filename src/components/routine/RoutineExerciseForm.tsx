          ...
          {exercise.isOpen && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.3 }} className="mt-3 space-y-3">
              <div className="grid grid-cols-1 gap-3">
                ...
                <div><label className="block text-[#E0E0E0] text-sm font-medium mb-1">Músculos Trabajados</label><Input name={`muscleGroup-${exercise._id}`} value={formatListField(exercise.muscleGroup)} onChange={(e) => onChange(dayIndex, exercise._id, "muscleGroup", e.target.value)} placeholder="Ej. Pecho, espalda baja" disabled={isCoachRestricted} className="w-full bg-[#2D2D2D] border border-[#4A4A4A] text-[#E0E0E0] placeholder-[#CCCCCC] rounded-lg p-3 text-sm focus:ring-2 focus:ring-[#34C759] focus:ring-offset-2 focus:ring-offset-[#1A1A1A] disabled:bg-[#2D2D2D]/80 disabled:opacity-75 disabled:cursor-not-allowed transition-colors min-h-12" /></div>
                <div><label className="block text-[#E0E0E0] text-sm font-medium mb-1">Consejos</label><Input name={`tips-${exercise._id}`} value={formatListField(exercise.tips)} onChange={(e) => onChange(dayIndex, exercise._id, "tips", e.target.value)} placeholder="Ej. Mantén la espalda recta, baja despacio" disabled={isCoachRestricted} className="w-full bg-[#2D2D2D] border border-[#4A4A4A] text-[#E0E0E0] placeholder-[#CCCCCC] rounded-lg p-3 text-sm focus:ring-2 focus:ring-[#34C759] focus:ring-offset-2 focus:ring-offset-[#1A1A1A] disabled:bg-[#2D2D2D]/80 disabled:opacity-75 disabled:cursor-not-allowed transition-colors min-h-12" /></div>
              </div>
